...
from heddle.envelope import ENRICHMENT_VOCAB
...
def test_mcp_tool_inventory_is_agent_first_and_enrich_only() -> None:
    ...
    assert fixture["schema"] == "heddle.mcp_tool_inventory.v1"
    ...
    for tool in tools:
        ...
        is_capture = tool["name"] in {"capture_snapshot", "heddle_edge_snapshot_capture"}
        ...
        assert tool["schema"].startswith("heddle.") and ".draft." not in tool["schema"]
...
def test_changed_response_fixture_is_frozen_envelope() -> None:
    ...
    assert fixture["schema"] == "heddle.change_list.v1"
