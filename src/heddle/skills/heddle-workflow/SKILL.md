...
description: >
  ...
  uses heddle for temporal change-impact tracking. heddle is the Weft federation's
  temporal authority; its MCP tools (mcp__heddle__*) answer change-impact and
  reverify questions, enrich-only, from a local-first dated store.
---

# Heddle Workflow

heddle is the federation's **temporal / change-impact authority** — "if I touch
...
- `heddle_change_list` / `changed` — changed entities for a repo + rev range.
  Returns `next_actions` pointing at reverify/impact. **Call this first.**
- `heddle_impact_radius_get` / `blast_radius` — downstream affected set over the
  dated snapshot. Carries mandatory `completeness` + `staleness`.
- `heddle_reverify_worklist_get` / `reverify` — the worklist to recheck before
  claiming completion. The flagship.
- `heddle_entity_timeline_get` / `timeline` — ordered change history for one
  entity; reports `sei_resolution` only, never lineage.
- `heddle_entity_churn_count_get` / `churn` — per-entity change-event counts.
- `heddle_edge_snapshot_capture` / `capture_snapshot` — the only mutating tool;
  captures dated loomweave edges into `.weft/heddle/`. Run when impact/reverify
...
- `enrichment` is a CLOSED vocab: `present` / `absent` (peer present, no fact) /
  ...
  implied clean/allowed state. heddle never gates — its facts are advisory.
...
- **`references/degrade-and-federation.md`** — completeness/staleness semantics,
  ...
  lifecycle. Load when an answer looks thin or when wiring heddle into a repo.
