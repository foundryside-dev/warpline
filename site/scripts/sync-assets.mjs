...
// (kit README "Copy the assets"). This runs automatically via the pre{dev,build}
// npm hooks. Resolved from the installed package first, falling back to the
// vendored copy fetched by fetch-site-kit.mjs (works pre-install).
...
if (!src) {
  ...
  console.error('[sync-assets] run `npm run fetch-site-kit` (or `npm install`) first.');
  process.exit(1);
}
